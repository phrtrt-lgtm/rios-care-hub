          ...
          <div className={isPrinting ? '' : 'h-64 w-full'}>
            {isPrinting ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 25, right: 10, left: 10, bottom: 10 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                  <XAxis dataKey="name" stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} />
                  <YAxis stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} tickFormatter={formatYAxis} />
    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="container max-w-5xl py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button variant="outline" onClick={handlePrint} className="gap-2"><Printer className="h-4 w-4" />Imprimir</Button>
                <Button onClick={handleExportPdf} disabled={isExporting} className="gap-2">
                  {isExporting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}Salvar PDF
                </Button>
              </div>
            </div>
          </div>
      ...
      <div className="container max-w-5xl py-8">
        <div ref={reportContentRef} className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
          {config.reportType === 'owner' ? (
            <>
              <div className="px-8 py-7 bg-foreground text-background">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="bg-background/10 rounded-xl p-2.5"><img src={logo} alt="Rios Hospedagens" className="h-9 w-auto" /></div>
                    <div><h1 className="text-xl font-bold text-background">Rios Hospedagens</h1><p className="text-sm text-background/60">Extrato do Proprietário</p></div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-xs font-semibold uppercase tracking-widest px-3 py-1 rounded-full border border-background/20" style={{ backgroundColor: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)' }}>Confidencial</span>
                    <div className="text-right text-sm"><p className="text-background/60">Emitido em</p><p className="font-semibold text-background">{formatReportDate(generatedAt)}</p></div>
                  </div>
                </div>
              </div>
              <div className="px-8 py-5 bg-primary/5 border-b border-border">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-lg bg-primary/15 flex items-center justify-center"><Home className="h-5 w-5 text-primary" /></div>
                  <div><p className="text-xs text-muted-foreground uppercase tracking-wider">Imóvel</p><p className="text-lg font-bold text-foreground">{config.propertyName}</p></div>
                  <div className="ml-auto text-right"><p className="text-xs text-muted-foreground uppercase tracking-wider">Período</p><p className="font-semibold text-foreground">{dateRangeText}</p></div>
                </div>
              </div>
            </>
          ) : (
            <>
              <div className="bg-primary px-8 py-6 text-primary-foreground">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="bg-primary-foreground rounded-lg p-2"><img src={logo} alt="Rios Hospedagens" className="h-8 w-auto" /></div>
                    <div><h1 className="text-xl font-bold">Rios Hospedagens</h1><p className="text-sm text-primary-foreground/80">Relatórios Financeiros</p></div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-xs font-semibold uppercase tracking-widest px-3 py-1 rounded-full border border-primary-foreground/20" style={{ backgroundColor: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)' }}>Confidencial</span>
                    <div className="text-right text-sm"><p className="text-primary-foreground/80">Gerado em</p><p className="font-medium">{formatReportDate(generatedAt)}</p></div>
                  </div>
                </div>
              </div>
              <div className="px-8 py-6 border-b border-border bg-muted/30">
                <div className="rounded-lg p-4 bg-muted/50">
                  <div className="grid gap-0 grid-cols-2 md:grid-cols-4">
                    <div className="px-4 first:pl-0"><p className="text-xs text-muted-foreground uppercase tracking-wider">Imóvel</p><p className="font-semibold text-foreground mt-1">{config.propertyName}</p></div>
                    <div className="px-4 border-l border-border"><p className="text-xs text-muted-foreground uppercase tracking-wider">Período</p><p className="font-medium text-foreground mt-1">{dateRangeText}</p></div>
                    <div className="px-4 border-l border-border"><p className="text-xs text-muted-foreground uppercase tracking-wider">Comissão Gestão</p><p className="font-semibold text-foreground mt-1">{config.commissionPercentage}%</p></div>
                    <div className="px-4 border-l border-border"><p className="text-xs text-muted-foreground uppercase tracking-wider">Tipo</p><p className="font-medium text-foreground mt-1">{getReportTypeLabel(config.reportType)}</p></div>
                  </div>
                </div>
              </div>
            </>
          ...
          {/* Executive Summary */}
          <div data-pdf-section="summary" className="px-8 py-8 border-b border-border">
            {config.reportType === 'owner' ? (
              <>
                <div className="flex items-center gap-3 mb-6">
                  <div className="p-2 bg-primary/10 rounded-lg"><TrendingUp className="h-5 w-5 text-primary" /></div>
                  <div><h2 className="text-lg font-semibold text-foreground">Resumo do Período</h2><p className="text-sm text-muted-foreground">Receita líquida das suas reservas</p></div>
                </div>
                ...
                <div className="grid gap-3 mb-8" style={{ gridTemplateColumns: 'repeat(3, minmax(0, 1fr))' }}>
                  {[
                    ...
                    const Icon = kpi.icon;
                    return (
                      <div key={i} className="rounded-xl border border-border bg-card p-4 relative" style={{ borderLeft: '4px solid #C0522B', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                        <div className="absolute top-3 right-3 rounded-lg p-1.5" style={{ backgroundColor: 'rgba(192, 82, 43, 0.08)' }}><Icon className="h-4 w-4" style={{ color: '#C0522B' }} /></div>
                      ...
                      {kpis.channels.map(ch => {
                        ...
                        return (
                          <div key={ch.name} className="flex items-center gap-3">
                            <span className="text-sm font-medium text-foreground w-24 shrink-0">{ch.name}</span>
                            <div className="flex-1 h-3 bg-muted rounded-full overflow-hidden"><div className="h-full bg-primary rounded-full" style={{ width: `${pct}%` }} /></div>
                            <span className="text-sm text-muted-foreground w-28 text-right shrink-0">{formatReportCurrency(ch.revenue)} ({pct}%)</span>
                          </div>
                        );
                      })}
                    </div>
                ...
                {kpis.bestMonth && monthGroups.length > 1 && (
                  <div className="rounded-xl border border-border bg-accent/10 p-4 mb-8 flex items-center gap-3">
                    <span className="text-2xl">🏆</span>
                    <div><p className="text-sm font-semibold text-foreground">Melhor mês: {kpis.bestMonth.monthLabel}</p><p className="text-xs text-muted-foreground">{kpis.bestMonth.totals.reservationCount} reservas · {formatReportCurrency(kpis.bestMonth.totals.totalOwnerNet)}</p></div>
                  </div>
                )}
                {renderChart()}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {monthGroups.map((group) => {
                    ...
                    return (
                      <div key={group.monthKey} className="rounded-xl border border-border bg-card p-4" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.07)' }}>
                        <div className="flex items-center gap-2 mb-3"><div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: '#C0522B' }} /><p className="font-semibold text-foreground">{group.monthLabel}</p></div>
                        <p className="text-2xl font-bold text-foreground">{formatReportCurrency(monthTotal.value)}</p>
                        <div className="flex gap-2 mt-2">
                          <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(192, 82, 43, 0.1)', color: '#C0522B' }}>{group.totals.reservationCount} reservas</span>
                          <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(192, 82, 43, 0.1)', color: '#C0522B' }}><span className="inline-flex items-center gap-1"><Moon className="h-3 w-3" />{monthNights} noites</span></span>
                        </div>
                      </div>
                    );
                  })}
              ...
              <>
                <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-primary/10 rounded-lg"><TrendingUp className="h-5 w-5 text-primary" /></div><div><h2 className="text-lg font-semibold text-foreground">Resumo Executivo</h2><p className="text-sm text-muted-foreground">Desempenho mês a mês</p></div></div>
                <div className="bg-primary rounded-xl p-6 text-primary-foreground mb-8">
                  <div className="flex items-center justify-between">
                    <div><p className="text-primary-foreground/80 text-sm">{getMainValueLabel()} - Total do Período</p><p className="text-3xl font-bold mt-1">{formatReportCurrency(getGrandTotal())}</p><p className="text-primary-foreground/70 text-sm mt-2">{totals.reservationCount} reservas</p></div>
                ...
                <div className="grid gap-3 mb-8" style={{ gridTemplateColumns: 'repeat(4, minmax(0, 1fr))' }}>
                  {[
                    ...
                    const Icon = kpi.icon;
                    return (
                      <div key={i} className="rounded-lg p-4 border border-border/50 bg-card relative" style={{ borderLeft: '4px solid #C0522B', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                        <div className="absolute top-3 right-3 rounded-lg p-1.5" style={{ backgroundColor: 'rgba(192, 82, 43, 0.08)' }}><Icon className="h-4 w-4" style={{ color: '#C0522B' }} /></div>
                        <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">{kpi.label}</p>
                        <p className="text-2xl font-bold text-foreground">{kpi.value}</p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {monthGroups.map((group) => {
                      const monthTotal = getMonthTotal(group.totals);
                      return (
                        <div key={group.monthKey} className="bg-muted/50 p-4 border border-border/50 rounded-xl" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.07)' }}>
                          <div className="flex items-center gap-2 mb-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: '#C0522B' }} /><p className="font-medium text-foreground">{group.monthLabel}</p></div>
                          <p className="text-2xl font-bold text-foreground">{formatReportCurrency(monthTotal.value)}</p>
                          <span className="inline-block text-xs font-medium px-2 py-0.5 rounded-full mt-2" style={{ backgroundColor: 'rgba(192, 82, 43, 0.1)', color: '#C0522B' }}>{group.totals.reservationCount} reservas</span>
                        </div>
                      );
                    })}
                  </div>
          ...
          {/* General Summary */}
          <div data-pdf-section="resumo-geral" className="px-8 py-6 border-b border-border">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {summary.map((item, idx) => {
                ...
                return (
                  <div key={idx} className={`rounded-lg p-4 relative ${item.highlight ? 'text-white' : 'bg-muted/50'}`}
                    ...
                    {!item.highlight && SummaryIcon && <div className="absolute top-3 right-3"><SummaryIcon className="h-5 w-5" style={{ color: '#C0522B', opacity: 0.3 }} /></div>}
                    <p className={`text-xs ${item.highlight ? 'text-white/80' : 'text-muted-foreground'}`}>{item.label}</p>
                    <p className={`text-xl font-bold mt-1 ${item.highlight ? 'text-white' : 'text-foreground'}`} style={{ fontVariantNumeric: 'tabular-nums' }}>{formatReportCurrency(item.value)}</p>
                    {item.highlight && <div className="flex items-center gap-1 mt-2" style={{ opacity: 0.8 }}><Users className="h-3.5 w-3.5 text-white" /><span className="text-xs text-white">{totals.reservationCount} reservas</span></div>}
                  </div>
                );
              })}
            </div>
          ...
          <div className="px-8 py-6">
            ...
            {monthGroups.map((group) => {
              const monthTotal = getMonthTotal(group.totals);
              const isOwner = config.reportType === 'owner';
              return (
                <div key={group.monthKey} data-pdf-section={`month-${group.monthKey}`} style={{ marginBottom: '32px' }}>
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-1 h-8 rounded-full" style={{ backgroundColor: '#C0522B' }} />
                      <Calendar className="h-5 w-5" style={{ color: '#C0522B' }} />
                      <div><h3 className="text-lg font-semibold text-foreground">{group.monthLabel}</h3><p className="text-sm text-muted-foreground">{group.totals.reservationCount} reservas</p></div>
                    </div>
                  </div>
                    ...
                    <table className="w-full table-fixed border-collapse" style={{ fontSize: '10px' }}>
                      <thead>
                        <tr style={{ backgroundColor: 'hsl(var(--muted))' }}>
                          {columns.map((col, idx) => (
                            <th key={idx} className={`px-1 py-1 font-semibold border-b border-border text-[10px] uppercase tracking-tight whitespace-nowrap ${isOwner ? (idx === columns.length - 1 ? 'text-right' : idx >= 3 ? 'text-right' : 'text-left') : (idx >= 2 ? 'text-right' : 'text-left')}`}
                          ...
                          return (
                            <tr key={reservation.id} className="border-b border-border/30 last:border-0" style={{ backgroundColor: rIdx % 2 === 0 ? 'transparent' : 'hsl(var(--muted) / 0.3)' }}>
                              {values.map((val, vIdx) => {
                                ...
                                return (
                                  <td key={vIdx} className={`px-1 py-1 text-[10px] ${isOwner ? (vIdx === 0 ? 'font-semibold text-foreground' : isLastCol ? 'text-right' : vIdx >= 3 ? 'text-right text-muted-foreground' : 'text-muted-foreground') : `${vIdx >= 2 ? 'text-right' : ''} ${vIdx === 0 ? 'font-medium text-foreground' : 'text-muted-foreground'}`}`}
                  ...
                  <div className="mt-3 flex justify-end">
                    <div className="inline-flex items-center gap-4 text-white" style={{ background: 'linear-gradient(135deg, #C0522B 0%, #A8431F 100%)', borderRadius: '8px', padding: '12px 20px', boxShadow: '0 2px 6px rgba(192,82,43,0.3)' }}>
                      <span className="text-sm font-semibold text-white/90">Total {group.monthLabel}:</span>
                      <span className="text-lg font-bold text-white">{formatReportCurrency(monthTotal.value)}</span>
                    </div>
                  </div>
                </div>
              );
          ...
          {config.reportType === 'owner' && (
            <div className="px-8 py-6 border-t border-border">
              ...
              {isPrinting ? (
                ...
                <textarea className="w-full rounded-lg border border-border bg-background px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40 resize-y min-h-[80px]" placeholder="Escreva uma observação para incluir no relatório…" value={observation} onChange={(e) => setObservation(e.target.value)} rows={3} />
              )}
            </div>
          )}
